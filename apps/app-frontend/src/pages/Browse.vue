...
import { install_create_modpack_instance } from '@/helpers/install'
...
function getCardActions(
	...
	if ((projectResult as any).is_curseforge || projectResult.project_id?.startsWith('cf_')) {
		return [
			{
				key: 'install',
				label: formatMessage(isInstalling ? messages.installingToServer : commonMessages.installButton),
				...
				onClick: async () => {
					setProjectInstalling(projectResult.project_id, true)
					try {
						const cfUrl = (projectResult as any).cf_download_url
						if (currentProjectType === 'modpack' && cfUrl) {
							const filePath = await invoke<string>('download_modpack_file', {
								url: cfUrl,
...
async function search(requestParams: string) {
	...
	const parsedParams = new URLSearchParams(requestParams)
	const queryText = parsedParams.get('query') || parsedParams.get('q') || searchState?.query?.value || ''

	console.group(`[BROWSE SEARCH] Query: "${queryText}" | Type: ${projectType.value}`)
	...
	let cfMappedHits: any[] = []

	if (!isServer && ['modpack', 'mod', 'resourcepack', 'shader'].includes(projectType.value)) {
		try {
			const cfItems = await searchCurseForge(queryText, projectType.value, 20)
			cfMappedHits = cfItems.map((cfItem) => {
				const latestFile = cfItem.latestFiles?.[0]
				return {
					project_id: `cf_${cfItem.id}`,
					id: `cf_${cfItem.id}`,
					slug: cfItem.slug,
					name: cfItem.name,
					title: cfItem.name,
					author: cfItem.authors?.[0]?.name ?? 'CurseForge',
					author_id: cfItem.authors?.[0]?.name ?? 'CurseForge',
					organization: null,
					organization_id: null,
					description: cfItem.summary,
					summary: cfItem.summary,
					categories: ['curseforge', ...(cfItem.categories?.map((c) => c.slug) ?? [])],
					display_categories: ['CurseForge', ...(cfItem.categories?.map((c) => c.name) ?? [])],
					versions: latestFile?.gameVersions ?? [],
					downloads: cfItem.downloadCount,
					follows: 0,
					icon_url: cfItem.logo?.thumbnailUrl ?? cfItem.logo?.url ?? '',
					date_created: new Date().toISOString(),
					date_modified: new Date().toISOString(),
					latest_version: latestFile?.displayName ?? '',
					license: 'CurseForge',
					client_side: 'optional',
					server_side: 'optional',
					project_type: projectType.value,
					gallery: [],
					featured_gallery: undefined,
					loaders: [],
					project_loader_fields: {},
					color: undefined,
					thread_id: '',
					monetization_status: 'monetized',
					cf_download_url: latestFile?.downloadUrl,
					cf_file_id: latestFile?.id,
					is_curseforge: true,
				}
			})
		} catch (e) {
			console.error('[CurseForge Search Exception]:', e)
		}
	}

	const modrinthHits = (rawResults?.result?.hits ?? []).map((hit) => ({
		...hit,
		display_categories: ['Modrinth', ...(hit.display_categories ?? hit.categories ?? [])],
	}))

	console.log(`[RESULTS SUMMARY] Modrinth: ${modrinthHits.length} items | CurseForge: ${cfMappedHits.length} items`)
	console.groupEnd()
	...
	if (isServer) {
		updateServerHits(modrinthHits)
		return {
			projectHits: [],
			serverHits: modrinthHits,
			total_hits: modrinthHits.length,
			per_page: rawResults?.result?.hits_per_page ?? 20,
		}
	}

	const mappedModrinthHits = modrinthHits.map((hit) => {
	...
	const combinedHits = [...mappedModrinthHits, ...cfMappedHits]

	const indexParam = parsedParams.get('index') || 'relevance'
	if (indexParam === 'newest') {
		combinedHits.sort((a, b) => new Date(b.date_created || 0).getTime() - new Date(a.date_created || 0).getTime())
	} else if (indexParam === 'updated') {
		combinedHits.sort((a, b) => new Date(b.date_modified || 0).getTime() - new Date(a.date_modified || 0).getTime())
	} else if (indexParam === 'follows') {
		combinedHits.sort((a, b) => (b.follows ?? 0) - (a.follows ?? 0))
	} else {
		// Sort by downloads descending by default (relevance / downloads)
		combinedHits.sort((a, b) => (b.downloads ?? 0) - (a.downloads ?? 0))
	}

	return {
		projectHits: combinedHits,
		serverHits: [],
		total_hits: (rawResults?.result?.total_hits ?? 0) + cfMappedHits.length,
		per_page: (rawResults?.result?.hits_per_page ?? 20) + cfMappedHits.length,
	}
}
...
const searchProvider = ref<'modrinth' | 'curseforge'>('modrinth')
