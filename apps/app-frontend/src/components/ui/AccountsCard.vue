	...
	>
		...
		<div class="flex flex-col gap-2">
			...
			<div class="flex items-center gap-2 my-1">
				<div class="h-px bg-surface-5 flex-grow"></div>
				<span class="text-xs text-text-muted">OR OFFLINE</span>
				<div class="h-px bg-surface-5 flex-grow"></div>
			</div>
			<form class="flex gap-2" @submit.prevent="createOfflineAccount">
				<input
					v-model="offlineUsername"
					type="text"
					placeholder="Offline Username"
					class="px-3 py-1.5 bg-bg-raised border border-surface-5 rounded-lg text-sm text-contrast w-full outline-none focus:border-primary"
				/>
				<ButtonStyled color="brand" type="outlined">
					<button type="submit" :disabled="!offlineUsername.trim() || loginDisabled">
						Add
					</button>
				</ButtonStyled>
			</form>
	...
	>
		...
		<div class="bg-button-bg pt-1 pb-2 border border-solid border-surface-5">
			...
			<div class="flex flex-col gap-2 px-2 pt-2">
				...
				<form class="flex gap-1.5 mt-1" @submit.prevent="createOfflineAccount">
					<input
						v-model="offlineUsername"
						type="text"
						placeholder="Offline Username"
						class="px-2.5 py-1 bg-bg-raised border border-surface-5 rounded-lg text-xs text-contrast w-full outline-none focus:border-primary"
					/>
					<ButtonStyled color="brand" type="outlined" size="small">
						<button type="submit" class="!px-2 !py-1 text-xs" :disabled="!offlineUsername.trim() || loginDisabled">
							+ Offline
						</button>
					</ButtonStyled>
				</form>
...
async function createOfflineAccount() {
	...
	try {
		const created = await offline_login(offlineUsername.value.trim())
		if (created) {
			await setAccount(created)
