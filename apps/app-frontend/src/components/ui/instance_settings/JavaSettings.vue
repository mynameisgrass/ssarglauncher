...
const installingJdk = ref(false)
const jdkInstallStatus = ref<string | null>(null)

async function handleAutoInstallJdk() {
	const version = optimalJava?.parsed_version ?? 21
	installingJdk.value = true
	jdkInstallStatus.value = `Downloading & Installing Java ${version} JDK...`
	try {
		const installedPath = await invoke<string>('plugin:jre|jre_auto_install_java', {
			javaVersion: version,
		})
		if (installedPath) {
			overrideJavaInstall.value = true
			javaPath.value = installedPath
			jdkInstallStatus.value = `Successfully installed Java ${version} JDK!`
		}
	} catch (err) {
		console.error('Failed to auto-install JDK:', err)
		jdkInstallStatus.value = `Install failed: ${err}`
		handleError(err as Error)
	} finally {
		installingJdk.value = false
	}
}
...
function applyGcPreset(type: 'zgc' | 'shenandoah' | 'aikar' | 'gen_zgc') {
	overrideJavaArgs.value = true
	if (type === 'zgc') {
		javaArgs.value = '-XX:+UseZGC -XX:+UnlockExperimentalVMOptions -XX:ZAllocationSpikeTolerance=5'
	} else if (type === 'shenandoah') {
		javaArgs.value = '-XX:+UseShenandoahGC -XX:+UnlockExperimentalVMOptions -XX:ShenandoahGCHeuristics=adaptive'
	} else if (type === 'aikar') {
		javaArgs.value = '-XX:+UseG1GC -XX:+UnlockExperimentalVMOptions -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M'
	} else if (type === 'gen_zgc') {
		javaArgs.value = '-XX:+UseZGC -XX:+ZGenerational -XX:+UnlockExperimentalVMOptions'
	}
}
...
<template>
	<div>
				...
				<div class="flex flex-col gap-2 flex-1 min-w-0">
					...
					<div class="flex flex-wrap gap-2 mt-2">
						<ButtonStyled color="brand">
							<button type="button" :disabled="installingJdk" @click="handleAutoInstallJdk">
								<SpinnerIcon v-if="installingJdk" class="animate-spin h-4 w-4" />
								<CoffeeIcon v-else class="h-4 w-4" />
								Auto-Install Java {{ optimalJava?.parsed_version ?? 21 }} JDK
							</button>
						</ButtonStyled>
						<ButtonStyled v-if="overrideJavaInstall">
							<button type="button" @click="handleDetectJava">
								<SearchIcon />
								Detect
							</button>
						</ButtonStyled>
						<ButtonStyled v-if="overrideJavaInstall">
							<button type="button" @click="handleBrowseJava">
								<FolderSearchIcon />
								Browse
							</button>
						</ButtonStyled>
					...
					<div
						v-if="jdkInstallStatus"
						class="text-xs font-semibold mt-2 px-3 py-2 rounded-lg"
						:class="[installingJdk ? 'bg-brand/10 text-brand animate-pulse' : 'bg-surface-3 text-contrast']"
					>
						{{ jdkInstallStatus }}
					</div>
				</div>
		...
		/>
		<div class="flex flex-wrap items-center gap-2 my-3 p-3 bg-surface-2 rounded-xl">
			<span class="text-xs font-semibold text-secondary mr-1">Suggested GC Flags:</span>
			<ButtonStyled color="standard" color-fill="bg">
				<button type="button" class="text-xs px-2.5 py-1" @click="applyGcPreset('zgc')">⚡ ZGC (Low Latency)</button>
			</ButtonStyled>
			<ButtonStyled color="standard" color-fill="bg">
				<button type="button" class="text-xs px-2.5 py-1" @click="applyGcPreset('shenandoah')">🌿 Shenandoah GC</button>
			</ButtonStyled>
			<ButtonStyled color="standard" color-fill="bg">
				<button type="button" class="text-xs px-2.5 py-1" @click="applyGcPreset('gen_zgc')">🚀 Generational ZGC</button>
			</ButtonStyled>
			<ButtonStyled color="standard" color-fill="bg">
				<button type="button" class="text-xs px-2.5 py-1" @click="applyGcPreset('aikar')">🔥 Aikar's Flags</button>
			</ButtonStyled>
		</div>
		<h2 class="mt-4 mb-1 text-lg font-extrabold text-contrast block">
