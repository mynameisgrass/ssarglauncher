...
async fn download_modpack_file(url: String, filename: String) -> api::Result<String> {
    ...
    let response = client.get(&url).send().await.map_err(theseus::Error::from)?;
    let bytes = response.bytes().await.map_err(theseus::Error::from)?;
...
async fn search_curseforge(
    ...
    let mut url = format!(
        "https://api.curseforge.com/v1/mods/search?gameId=432&classId={}&pageSize={}&index={}",
        class_id,
    ...
    if trimmed.is_empty() {
        url.push_str("&sortField=6&sortOrder=desc");
    } else {
