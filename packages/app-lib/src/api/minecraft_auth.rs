...
/// Create and set an offline / cracked Minecraft account credentials
#[tracing::instrument]
pub async fn offline_login(username: &str) -> crate::Result<Credentials> {
    let state = State::get().await?;
    let username = username.trim();
    if username.is_empty() {
        return Err(crate::ErrorKind::OtherError("Username cannot be empty".to_string()).as_error());
    }
    ...
    let offline_uuid = uuid::Uuid::from_bytes(bytes);
